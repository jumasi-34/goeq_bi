...
def validate_dataframe(df: pd.DataFrame) -> bool:
    ...
    try:
        ...
        if not pd.api.types.is_numeric_dtype(df["SUPP_QTY"]):
            print("SUPP_QTY 컬럼이 숫자형이 아닙니다.")
            return False

        # NULL 값 검증
        null_counts = df.isnull().sum()
        if null_counts.any():
            print(f"NULL 값이 존재합니다:\n{null_counts[null_counts > 0]}")

        # 음수 수량 검증
        negative_qty = df[df["SUPP_QTY"] < 0]
        if not negative_qty.empty:
            print(f"음수 수량이 {len(negative_qty)}건 존재합니다.")
...
def save_df_to_sqlite(
    ...
    try:
        ...
        if not db_path.parent.exists():
            ...
            print(f"DB 디렉토리 생성: {db_path.parent}")

        with sqlite3.connect(DB_PATH) as conn:
            df.to_sql(table_name, conn, if_exists=if_exists, index=index)
            print(f"테이블 '{table_name}' 저장 완료 (레코드 수: {len(df)})")
        return True

    except Exception as e:
        print(f"SQLite 저장 중 오류 발생: {str(e)}")
        return False
...
def generate_sellin_monthly_agg() -> tuple[bool, str]:
    ...
    try:
        ...
        try:
            ...
            print(f"Oracle DB에서 {len(df)}건의 데이터 조회 완료")
        except Exception as e:
