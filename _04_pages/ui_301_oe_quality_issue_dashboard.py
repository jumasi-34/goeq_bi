...
def render_global_tab(data, selected_year):
    ...
    cols[0].plotly_chart(
        viz.draw_three_years_oeqi(data['global_yearly']), 
    ...
    cols = st.columns(3)
    cols[0].plotly_chart(
        viz.draw_supply_quantity_by_plant(data['plant_yearly'])
    )
    cols[1].plotly_chart(
        viz.draw_issue_count_by_plant(data['plant_yearly'])
    )
    cols[2].plotly_chart(
        viz.draw_oeqi_by_plant(data['plant_yearly'])
    )
    ...
    cols[1].plotly_chart(
        viz.draw_mttc_global_indicator(df_mttc), 
        use_container_width=True
    )
    cols[2].plotly_chart(
        viz.draw_mttc_by_plant(data['plant_yearly']), 
        use_container_width=True
    )
    ...
    cols = st.columns(4)
    cols[0].plotly_chart(
        viz.draw_mttc_reg_global_indicator(df_mttc), 
        use_container_width=True
    )
    cols[1].plotly_chart(
        viz.draw_mttc_rtn_global_indicator(df_mttc), 
        use_container_width=True
    )
    cols[2].plotly_chart(
        viz.draw_mttc_countermeasure_global_indicator(df_mttc), 
        use_container_width=True
    )
    cols[3].plotly_chart(
        viz.draw_mttc_8d_global_indicator(df_mttc), 
        use_container_width=True
    )
...
def render_plant_tab(data, selected_year):
    ...
    with cols[0]:
        ...
        # 데이터가 없는 경우 0으로 처리
        mass_prod_count = df_plt_projects[df_plt_projects["Status"] == "Supplying"].shape[0] if not df_plt_projects.empty else 0
        dev_count = df_plt_projects[df_plt_projects["Status"] == "Developing"].shape[0] if not df_plt_projects.empty else 0
    ...
    with cols[1]:
        ...
        supp_qty = data['plant_yearly'].loc[
            data['plant_yearly']["PLANT"] == selected_plt, "SUPP_QTY"
        ].values[0]
    ...
    with cols[3]:
        ...
        oeqi_val = data['plant_yearly'].loc[
            data['plant_yearly']["PLANT"] == selected_plt, "OEQI"
        ].values[0]
        mttc_val = data['plant_yearly'].loc[
            data['plant_yearly']["PLANT"] == selected_plt, "MTTC"
        ].values[0]
    ...
    with cols[0]:
        ...
        st.plotly_chart(
            viz.draw_plant_view_issue_count_highlight(data['plant_yearly'], selected_plt),
            use_container_width=True,
        )
...
def render_oeqg_tab(data, selected_year):
    ...
    cols[1].plotly_chart(
        viz.draw_goeq_view_oeqi_monthly_trend(data['goeq_monthly'])
    )
    ...
    def draw_target_line(fig_local, target):
        fig_local.add_hline(
            y=target, 
            line=dict(color=config_plotly.NEGATIVE_CLR, dash="dash")
        )
...
def render_rawdata_tab(data, selected_year):
    ...
    columns = {
        ...
        'Classification': ['LOCATION', 'KPI', 'MARKET', 'M_CODE', 'PNL_NM', 'TYPE', 'CAT', 'SUB_CAT'],
    ...
    column_config = {
        ...
        "RESP_ID": st.column_config.TextColumn("Responsible", help="Quality issue responsible person"),
        
        # Date Information
        "OCC_DATE": st.column_config.DateColumn("Occurrence Date", format="YYYY-MM-DD", help="Quality issue occurrence date"),
        "REG_DATE": st.column_config.DateColumn("Registration Date", format="YYYY-MM-DD", help="Quality issue registration date"),
        "RTN_DATE": st.column_config.DateColumn("Return Date", format="YYYY-MM-DD", help="Quality issue return date"),
        "CTM_DATE": st.column_config.DateColumn("Countermeasure Date", format="YYYY-MM-DD", help="Countermeasure implementation date"),
        "COMP_DATE": st.column_config.DateColumn("Completion Date", format="YYYY-MM-DD", help="8D report completion date"),
        
        # Status Information
        "STATUS": st.column_config.TextColumn("Status", help="Current quality issue status"),
        "RETURN_YN": st.column_config.TextColumn("Return Status", help="Quality issue return status"),
        "HK_FAULT_YN": st.column_config.TextColumn("HK Fault", help="HK fault status"),
        
        # Period Information
        "REG_PRD": st.column_config.NumberColumn("Registration Period", help="Registration period in days", format="%.0f"),
        "RTN_PRD": st.column_config.NumberColumn("Return Period", help="Return period in days", format="%.0f"),
        "CTM_PRD": st.column_config.NumberColumn("Countermeasure Period", help="Countermeasure period in days", format="%.0f"),
        "COMP_PRD": st.column_config.NumberColumn("Completion Period", help="8D report completion period in days", format="%.0f"),
        "MTTC": st.column_config.NumberColumn("Total Period", help="Total period in days", format="%.0f"),
        
        # Other Information
        "URL": st.column_config.LinkColumn("Link", help="Link to quality issue detail page", display_text="Link"),
    }

    # 데이터프레임 표시
    filtered_data = data['raw_3_years'][data['raw_3_years']["YYYY"] == selected_year][remain_col]
    st.dataframe(
        filtered_data,
        use_container_width=True,
        hide_index=True,
